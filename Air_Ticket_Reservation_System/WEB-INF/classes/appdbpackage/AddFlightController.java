        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(source == null ? destination == null : source.equals(destination)) {
                JOptionPane.showMessageDialog(null, "Source and Destination cannot be the same.Please change the source and destination","Message", JOptionPane.INFORMATION_MESSAGE);
                 String url = "/Addflights.jsp";
                 getServletContext().getRequestDispatcher(url).forward(request, response); 
            }
            
            else {
            fdb.AddFlight(flightnum,source,destination,flighttype,arrival,departure);
             JOptionPane.showMessageDialog(null, "Flight Details Added","Congratulations", JOptionPane.INFORMATION_MESSAGE);
